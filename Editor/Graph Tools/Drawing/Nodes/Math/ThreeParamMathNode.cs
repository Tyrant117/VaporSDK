...
{
    public class ThreeParamMathNode<T> : GraphToolsNode<T, MathNodeSo> where T : ScriptableObject
    {
        private Port _in1Port;
        private Port _in2Port;
        private Port _in3Port;
        private Port _outPort;

        private List<string> _inPortNames;

        public ThreeParamMathNode(GraphEditorView<T> view, MathNodeSo node)
        {
            View = view;
            Node = node;
            FindParams();
        ...
        private void CreateFlowInPort()
        {
            ...
            _in3Port = InstantiatePort(Orientation.Horizontal, Direction.Input, Port.Capacity.Single, typeof(float));
            _in3Port.portName = _inPortNames.Count > 2 ? _inPortNames[2] : "C (1)";
            _in3Port.tooltip = "The flow input";
            Ports.Add(_in3Port);
            inputContainer.Add(_in3Port);
        }
